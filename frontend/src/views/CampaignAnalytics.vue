      ...
      <div class="chart" v-for="(v, k) in charts" :key="k">
        <div class="columns">
          <div class="column is-9">
            ...
            <line-chart v-if="!v.loading" :data="v.data" />
          </div>
          <div class="column is-2 donut-container">
            <donut-chart v-if="!v.loading" :data="v.donutData" />
          </div>
        </div>
      </div>
    </section>
...
export default Vue.extend({
  ...
  methods: {
    ...
    makeCharts(typ, campaigns, data) {
      ...
      return { lines: { datasets: lines }, donut };
    },
    ...
    getData(typ, camps) {
      ...
      this.charts[typ].fn({
        ...
        const { lines, donut } = this.charts[typ].chartFn(typ, camps, data);
        this.charts[typ].data = lines;
        this.charts[typ].donutData = donut;

        if (this.charts[typ].donutFn && this.settings['privacy.individual_tracking']) {
          this.charts[typ].donutData = this.charts[typ].donutFn(typ, camps, data);
        }
